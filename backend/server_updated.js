...
// GET - جلب الجمل (للمستخدم وللآخرين)
app.get('/api/sentences', protect, async (req, res) => {
  try {
    const { view } = req.query; // ?view=my أو ?view=all أو ?view=others
    const userId = req.user.id;

    let query = {};

    // تحديد نوع الجمل المطلوب عرضها
    if (view === 'my') {
      // جلب جمل المستخدم فقط
      query.userId = userId;
    } else if (view === 'others') {
      // جلب جمل المستخدمين الآخرين فقط
      query.userId = { $ne: userId };
    }
    // default: جلب كل الجمل (جمل المستخدم + جمل الآخرين)

    const sentences = await Sentence.find(query).sort({ createdAt: -1 });
    
    // إضافة معلومة الملكية وحساب الإحصائيات
    const sentencesWithStats = sentences.map(s => {
      const stats = calculateSentenceStats(s);
      return {
        ...s.toObject(),
        stats,
        isOwner: s.userId.toString() === userId.toString(), // علامة للملكية
        canEdit: s.userId.toString() === userId.toString(), // هل يمكن التعديل؟
        canDelete: s.userId.toString() === userId.toString() // هل يمكن الحذف؟
      };
    });
...
// GET - جلب جملة واحدة بالـ ID
app.get('/api/sentences/:id', protect, async (req, res) => {
  try {
    const userId = req.user.id;
    ...
    // إضافة معلومة الملكية
    const stats = calculateSentenceStats(sentence);
    const result = {
      ...sentence.toObject(),
      stats,
      isOwner: sentence.userId.toString() === userId.toString(),
      canEdit: sentence.userId.toString() === userId.toString(),
      canDelete: sentence.userId.toString() === userId.toString()
    };
...
// POST - مراجعة الجملة بنظام SM-2 (للمالك فقط)
...
// GET - الجمل المستحقة للمراجعة (للمستخدم فقط)
